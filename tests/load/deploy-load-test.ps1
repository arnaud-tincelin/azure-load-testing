...
# Update config file with actual target host in a temp directory alongside the JMX file
$configContent = Get-Content $configFile -Raw
$configContent = $configContent -replace '\$\{SERVICE_ALBUMS_API_ENDPOINT_URL\}', $TargetHost
$tempDir = Join-Path ([System.IO.Path]::GetTempPath()) "load-test-$(Get-Random)"
New-Item -ItemType Directory -Path $tempDir -Force | Out-Null
$tempConfig = Join-Path $tempDir "load-test.yaml"
$configContent | Set-Content $tempConfig -NoNewline
Copy-Item $testPlanFile $tempDir

# Create or update the load test using the config file
Write-Host "`nCreating/updating load test '$testId'..." -ForegroundColor Yellow
$existing = az load test show --test-id $testId --load-test-resource $LoadTestingResourceName --resource-group $ResourceGroup 2>$null
if ($existing) {
    az load test update `
        --test-id $testId `
